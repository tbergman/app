...
if [ -z "${TRAVIS_PULL_REQUEST}" ] || [ "${TRAVIS_PULL_REQUEST}" == "false" ]; then
    if [ "${TRAVIS_BRANCH}" == "develop" ]; then
        ...
        sed -i -e 's/https:\/\/gateway.test.hedvig.com/http:\/\/hedvig.ngrok.io/g' ./hedvig-redux/services/environment.js
        sed -i -e 's/"slug": "hedvig-app"/"slug": "hedvig-ngrok-app"/g' app.json
        exp logout
        exp login -u ${EXPO_DEV_USERNAME} -p ${EXPO_DEV_PASSWORD}
        exp publish --non-interactive
        exp logout
        sed -i -e 's/http:\/\/hedvig.ngrok.io/https:\/\/gateway.test.hedvig.com/g' ./hedvig-redux/services/environment.js
